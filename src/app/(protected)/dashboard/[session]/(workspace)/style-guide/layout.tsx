            ...
            <div className='mt-36 container mx-auto px-4 sm:px-6 py-6 sm:py-8'>
                <div >
                    <div className='flex flex-col lg:flex-row gap-4 lg:gap-5 items-center justify-between'>
                        ...
                        <TabsList className='grid w-full sm:w-fit h-auto grid-cols-3 rounded-full backdrop-blur-xl bg-white/8 border border-white/12 saturate-150'>
                            {tabs.map((tab) => {
                                const Icon = tab.icon
                                return (
                                    <TabsTrigger key={tab.value} value={tab.value} className="flex items-center gap-2 rounded-xl data-[state=active]:bg-white/15 data-[state=active]:backdrop-blur-xl data-[state=active]:border data-[state=active]:border-white/2 transition-all duration-200 text-xs sm:text-sm">
