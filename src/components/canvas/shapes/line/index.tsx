...
export const Line = ({ shape }: { shape: LineShape }) => {
    ...
    const minX = Math.min(startX, endX) - strokeWidth / 2 - 2;
    const minY = Math.min(startY, endY) - strokeWidth / 2 - 2;
    const maxX = Math.max(startX, endX) + strokeWidth / 2 + 2;
    const maxY = Math.max(startY, endY) + strokeWidth / 2 + 2;
