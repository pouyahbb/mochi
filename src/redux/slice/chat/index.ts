...
const chatSlice = createSlice({
    ...
    reducers : {
        ...
        startStreamingResponse : (state , action:PayloadAction<{
            generatedUIId : string
            messageId : string
        }>) => {
        ...
        updateStreamingContent : (state , action : PayloadAction<{
            ...
            const {content ,generatedUIId ,messageId} = action.payload
            const chat = state.chats[generatedUIId]
            if(chat){
        ...
        finishStreamingResponse : (state , action : PayloadAction<{
            generatedUIId : string
            messageId : string
            finalContent : string
        }>) => {
            const {finalContent ,generatedUIId ,messageId} = action.payload
            const chat = state.chats[generatedUIId]
            if(chat){
        ...
        addErrorMessage : (state , action : PayloadAction<{
            generatedUIId : string
            error: string
        }>) => {
...
export const {addErrorMessage ,addUserMessage ,clearChat ,finishStreamingResponse ,initializeChat ,startStreamingResponse ,updateStreamingContent} = chatSlice.actions
